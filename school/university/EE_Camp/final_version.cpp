...
int main() {
	...
	while (mined_cnt < SIZE*SIZE - BOMB_NUM) {
		...
		CleanScreen();
		output(graph, vis, mined_cnt, flag_cnt, cursor_row, cursor_col);
		while (ch == '\0') ch = get_choice();

		if ((ch == 'w' || ch == 65) && check (graph, cursor_row-1, cursor_col)) cursor_row--;
		else if ((ch == 's' || ch == 66) && check (graph, cursor_row+1, cursor_col)) cursor_row++;
		else if ((ch == 'a' || ch == 68) && check (graph, cursor_row, cursor_col-1)) cursor_col--;
		else if ((ch == 'd' || ch == 67) && check (graph, cursor_row, cursor_col+1)) cursor_col++;
		else if (ch == 'j') { //mine
			if (vis[cursor_row][cursor_col] == 1) continue;
			else if (graph[cursor_row][cursor_col] == -1) { //stepped on bomb
