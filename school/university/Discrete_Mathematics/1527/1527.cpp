...
bool change (int col, vector<int> &temp) {
	for (auto k: ver_of_col[col]) {
		...
		for (int i = 0; i < chromatic_num; i++) {
			if (i == col || !ver_of_col[i].size()) continue;
			bool valid = true;
...
int main() {
	...
	while (1) {
		bool changed = false;
		for (int i = 0; i <= max_node; i++) ver_of_col[i].clear();
		for (int i = 0; i <= max_node; i++) ver_of_col[color[i]].push_back (i);

		for (int i = 0; i < chromatic_num; i++) {
			if (ver_of_col[i].empty()) continue;
			temp = color;
			if (change(i, temp)) {
				changed = true;
				color = temp;
	...
	for (int i = chromatic_num-1; i >= 0; i--) {
		if (!ver_of_col[i].size()) chromatic_num--;
	}
	cout << chromatic_num << '\n';

	for (int i = 0; i <= max_node; i++) cout << i << "-" << color[i]+1 << '\n';
